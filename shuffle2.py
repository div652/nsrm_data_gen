...
with open('instructions-train-shuffled.json', 'r') as file:
    data_instructions = json.load(file)

# Load data from scenes.json
with open('scenes-train-shuffled.json', 'r') as file:
...
assert(array_size>5000)

permutation=[]
for i in range (5000,array_size):
...
with open('instructions-train-shuffled2.json', 'w') as file:
    json.dump(data_instructions, file,indent=2)

# Save the updated data back to scenes.json
with open('scenes-train-shuffled2.json', 'w') as file:
