...
class Cube():
  def __init__(self, bullet, flags, position, color_idx = 1, orn = None) -> None:
    ...
    objct = bullet.loadURDF(Objects.Block.value, position, orn, flags=flags, globalScaling=1.0)
...
class Dice():
  def __init__(self, bullet, flags, position, color_idx = 1, orn = None) -> None:
    orn = bullet.getQuaternionFromEuler([0, 0, np.random.normal(0, math.pi/24, 1)[0]]) if orn is None else orn
    objct = bullet.loadURDF(DiceUrdf[color_idx].value, position, orn, flags=flags, globalScaling=1.0)
    self.type = 'Dice'
    self.object_idx = objct
    self.color = (color_idx, Color(color_idx).name)
    self.position = list(position)
...
class Lego():
  def __init__(self, bullet, flags, position, color_idx = 1, orn = None) -> None:
    orn = bullet.getQuaternionFromEuler([0, 0, np.random.normal(0, math.pi/24, 1)[0]]) if orn is None else orn
    objct = bullet.loadURDF(Objects.Lego.value, position, orn, flags=flags, globalScaling=1.67) ## 1.25*(31/18.5) 1.25 = object_dim for global scalling 1
    bullet.changeVisualShape(objct, -1, rgbaColor=ColorList[color_idx])
    self.type = 'Lego'
    self.object_idx = objct
    self.color = (color_idx, Color(color_idx).name)
...
class Tray():
  def __init__(self, bullet, flags, position, color_idx = 1, orn = None) -> None:
    objct = bullet.loadURDF(Objects.Tray.value, position, flags=flags, globalScaling=1)
    bullet.changeVisualShape(objct, -1, rgbaColor=ColorList[color_idx])
    self.type = 'Tray'
    self.object_idx = objct
    self.color = (color_idx, Color(color_idx).name)
